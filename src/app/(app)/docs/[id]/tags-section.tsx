...
interface DocTagsSectionProps {
  docId: string;
  initialTags: Tag[];
  canEdit: boolean;
}

export function DocTagsSection({
  docId,
  initialTags,
  canEdit,
}: DocTagsSectionProps) {
  ...
  const handleAdd = async (tagName: string) => {
    try {
      const response = await fetch(`/api/docs/${docId}/tags`, {
        method: "POST",
  ...
  const handleRemove = async (tagId: string) => {
    ...
    try {
      const response = await fetch(`/api/docs/${docId}/tags/${tagId}`, {
        method: "DELETE",
      });
